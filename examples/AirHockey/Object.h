...
public:
	...
	void SetRestitution(double f) {
		fd.restitution = f;
	}
	void SetDensity(double f) {
		fd.density = f;
	}
	void SetSensor(bool b = true) {
		fd.isSensor = b;
	}
	void FilterAllCollision() {
	...
	void SetSpeed(const Pointf& s) {
		body->SetLinearVelocity(b2Vec2(s.x, s.y));
	}
	void SetSpeed(double x, double y) {
		body->SetLinearVelocity(b2Vec2(x, y));
	}

	void ApplyForceToCenter(const Pointf& f) {
		body->ApplyForce(b2Vec2(f.x, f.y), body->GetLocalCenter());
	}

	bool IsColliding(const Object& o) const;
	bool IsDynamic() const {
		return body ? body->GetType() == b2_dynamicBody : bd.type == b2_dynamicBody;
	}
	...
	Pointf GetSpeed() const {
		b2Vec2 vel = body->GetLinearVelocity();
		return Pointf(vel.x, vel.y);
	}
...
public:
	...
	Polygon& AddVertex(double x, double y) {
		vertices.Add().Set(x, y);
		return *this;
	}
	Polygon& AddVertex(Pointf pt) {
		vertices.Add().Set(pt.x, pt.y);
		return *this;
	}
	Polygon& operator << (Pointf pt) {
		vertices.Add().Set(pt.x, pt.y);
		return *this;
	}
	...
	Pointf operator[](int i) {
		b2Vec2& b = vertices[i];
		return Pointf(b.x, b.y);
	}
