...
/* glmBounds: Calculates the bounds (min/max of XYZ) of
 * a model.
 *
...
glmDimensions(GLMmodel *model, GLfloat *dimensions)
{
   ...
   /* calculate model width, height, and depth */
   dimensions[0] = glmAbs(maxs[0]) + glmAbs(mins[0]);
   dimensions[1] = glmAbs(maxs[1]) + glmAbs(mins[1]);
   dimensions[2] = glmAbs(maxs[2]) + glmAbs(mins[2]);
}


/* glmHeightmap: Calculates a facet heightmap for a model.
 * Height is defined as the Y dimension on the XZ plane.
 * The heightmap is returned as a quadtree for efficient
 * searching. The application is responsible for freeing
 * the map memory.
 *
 * model      - initialized GLMmodel structure
 */
